      ...
      <Card className="bg-gradient-to-br from-pink-50 to-rose-50 border-pink-100">
        ...
        {!isCollapsed && (
          <CardContent className="space-y-6">
            ...
            {(stats.moodDistribution && stats.moodDistribution.length > 0) || stats.avgEnergyLevel ? (
              <div className="p-4 rounded-xl bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-100">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-semibold text-gray-800">Your Mood & Energy</h4>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {stats.moodDistribution && stats.moodDistribution.length > 0 && (
                    <div className="bg-white/60 rounded-lg p-3">
                      <p className="text-xs text-gray-500 mb-2">Top Mood This Week</p>
                      <div className="flex items-center gap-2">
                        <span className="text-2xl">{stats.moodDistribution[0].emoji}</span>
                        <div>
                          <span className="font-semibold text-gray-800 capitalize">
                            {stats.moodDistribution[0].mood}
                          </span>
                          <span className="text-xs text-gray-500 block">
                            {stats.moodDistribution[0].count} days
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Energy Level */}
                  {stats.avgEnergyLevel !== undefined && stats.avgEnergyLevel > 0 && (
                    <div className="bg-white/60 rounded-lg p-3">
                      <p className="text-xs text-gray-500 mb-2">Avg Energy Level</p>
                      <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-500 flex items-center gap-1">
                            ...
                            {stats.energyTrend === 'down' && (
                              <>
                                ...
                                <span className="text-red-600">Dipping</span>
                              </>
                            )}
                ...
                {stats.recentMoods && stats.recentMoods.filter(m => m.mood).length > 0 && (
                  <div className="mt-3 flex gap-1 justify-center">
                    {stats.recentMoods.slice(0, 7).map((m, i) => {
                      ...
                      return (
                        <div 
                          key={i} 
                          className="w-6 h-6 rounded-full bg-white/80 flex items-center justify-center text-sm"
                          title={m.mood ? `${m.mood} - ${new Date(m.date).toLocaleDateString()}` : 'No check-in'}
                        >
