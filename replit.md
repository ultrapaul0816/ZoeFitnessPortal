...
**Forgot Password / OTP Login**: Users who forget their password can request a 6-digit OTP code via email. After verification, they can either log in immediately or set a new password. OTP codes expire in 10 minutes and are stored in the `password_reset_codes` table. The system auto-accepts terms/disclaimer when logging in via OTP to streamline the process.
