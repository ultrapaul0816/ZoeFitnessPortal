...
- **11 Database Tables**: courses, course_modules, course_module_mappings, module_sections, content_items, course_enrollments, user_module_progress, user_content_completion, exercises, **structured_workouts**, **workout_exercise_links**
- **Admin Course Builder** (`/admin/courses`): Enterprise-grade interface with visible action buttons (Preview, Modules, Edit, Publish/Unpublish, Archive) instead of dropdown menus. Shows course creation dates and timestamps. Courses use standardized `durationWeeks` (integer) instead of free text for consistency.
- **Course Editor** (`/admin/courses/:courseId`): Interface for assigning and ordering modules within a course. Features checkbox multi-select for adding modules, visual ordering with drag handles, and quick access to module sections. Includes course image upload via Cloudinary and a Preview Course button.
- **Course Preview** (`/admin/courses/:courseId/preview`): Full preview of how a course will appear to users, including expandable modules and sections with all content items displayed. Includes a Content Audit tab showing completeness status with statistics (total modules, sections, items) and visual indicators for empty modules/sections.
...
- **Content Item Editor**: Supports 5 content types - Videos (YouTube URLs), Text/Articles, PDF Downloads, Exercises (linked from library), and Structured Workouts. Each content item has title, description, duration, and type-specific data fields. Features colored gradient headers (pink for Add, blue for Edit) for clear visual distinction.
- **Structured Workout Builder** (`/admin/workouts`): Complete workout builder supporting warmup/main workout/cooldown phases, multiple rounds, exercise sequences with drag-and-drop ordering. Each workout can define rounds, rest between exercises, rest between rounds, and total duration. Workouts can be assigned to course sections via the content item editor.
