...
**Today's Workout Feature**: Offers an immersive workout experience with program overview, inline YouTube video playback (videos open in modal within the app instead of external browser), prominent display of reps/duration, exercise-by-exercise tracking, "Zoe's Personalized Check-in" for limited workout swaps, workout difficulty star rating, a weekly progress bar, and a "Tomorrow Preview." First-time users are greeted with a welcome screen (shown once, tracked in database). Rest days show an encouraging message with an option to workout anyway (2-second loader with encouraging text before showing workout).
