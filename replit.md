...
2. **Private Coaching** (`private_coaching`): Uses a single comprehensive `private_coaching_questionnaire` form with 9 sections (A-I): basics, goal selection, medical details, lifestyle, nutrition, diet details, fitness history, exercise routine, goal setting, miscellaneous, and participant release (~80+ fields). AI prompts focus on personal fitness transformation, fat loss, muscle toning, etc. based on client goals.

Database tables: `coaching_clients`, `coaching_workout_plans`, `coaching_nutrition_plans`, `coaching_tips`, `direct_messages`, `coaching_checkins`, `coaching_form_responses`, `coaching_workout_completions`. Admin enrollment flow: Admin enrolls client (selecting coaching type) → Client fills in-app intake forms → Admin approves intake → AI generates 4-week workout plan and nutrition plan using OpenAI GPT-4o with coaching-type-appropriate prompts → Admin reviews/approves plan → Client status becomes active. Client statuses: enrolled → intake_complete → plan_generating → plan_ready → active → paused → completed → cancelled. Client-facing page at `/my-coaching` redesigned as a "High Performance Operating System" with 5 views (Today, Workouts, Nutrition, Messages, Check-in) navigated via a floating pill bar. The **Today view** (default) features a Profile Card (initials avatar, stats grid with age/height/weight/medical clearance from questionnaire data, constraint tags, primary goal, current cycle bar) and a vertical timeline showing the day's plan (Morning Ritual, Workout, Nutrition Strategy, Snack, Mindset & Recovery). The Workouts view has week pills and day accordion with exercise completion tracking. The Nutrition view shows a strategy card and meal option cards. Messages and Check-in views are preserved. Admin coaching management at `/admin/coaching` in the Coaching sidebar section.

### Workout Plan Exercise Structure
Coaching workout exercises use numeric seconds for all time-based fields to enable future countdown timer UI. Key fields per exercise: `durationSeconds` (number|null, in seconds), `reps` (string|null, e.g. "12", "8-10"), `restAfterSeconds` (number|null, in seconds). Validation: every exercise must have at least one of `durationSeconds` or `reps`. Section-level fields: `durationSeconds` (number|null), `restBetweenRoundsSeconds` (number|null). Warm-up and cool-down flows allow `exerciseId` and `videoUrl` to be null (videos will be added later). The frontend supports backward compatibility with legacy text-based `duration`/`restAfter` fields from older AI-generated plans.
...
The `coaching_form_responses` table stores in-app form submissions linked to coaching clients. Three form types: `lifestyle_questionnaire` (30+ fields covering personal info, pregnancy details, medical history, discomfort, movement history, goals, referral, consent), `health_evaluation` (personal info, participant declaration, doctor/midwife clearance with name, qualification, clinic, clearance decision, restrictions), and `private_coaching_questionnaire` (9 sections A-I with ~80+ fields: basics, goal selection, medical details, lifestyle, nutrition, diet details, fitness history, exercise routine, goal setting, miscellaneous, participant release). Admin can enter/edit form data via dialogs in the coaching client detail view. The UI conditionally renders forms based on the client's `coachingType` — private coaching shows a single comprehensive form, pregnancy coaching shows the existing two-form setup. API: `GET/POST /api/admin/coaching/clients/:clientId/form-responses`. Component: `client/src/components/admin/coaching-form-responses.tsx`.
